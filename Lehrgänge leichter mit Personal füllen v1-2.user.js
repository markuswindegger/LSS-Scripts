...
(async function() {
    ...
    var feuerwehrAusbauten = ["Rettungsdienst-Erweiterung","Wasserrettungs-Erweiterung","Flughafen-Erweiterung","Großwache","Werkfeuerwehr","Abrollbehälter-Stellplatz","Lager"];
        ...
        filtered = buildings.filter((e) => {
            if(e.extensions){
                return (document.getElementById('gebaeudeArt').value == "" || e.building_type == parseInt(oldGebäudeSelection)) && (document.getElementById('minPerson').value == "" || parseInt(document.getElementById('minPerson').value) <= e.personal_count) &&
                    (document.getElementById('ausbau').value == "" || document.getElementById('ausbau').value == "Lager" && e.storage_upgrades.length > 0 || e.extensions.filter((e1) => e1.caption.indexOf(document.getElementById('ausbau').value) >= 0).length > 0);
            } else {
                return false;
            }
        });
